...
sem_t buffer_mutex; // bsem mutex
sem_t gate_mutex; // wait mutex
...
void *producer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        ...
        count = count  - 1;
        insertbuffer(value);
        printf("Producer %d added %d to buffer, count is %d \n", thread_numb, value, count);
        if (count > 0) {
         sem_post(&gate_mutex);
        }
        sem_post(&buffer_mutex); /* protecting critical section */
...
void *consumer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        ...
        printf("Consumer %d dequeue %d from buffer, count is %d \n", thread_numb, value, count);
        if (count == 0) {
            sem_post(&gate_mutex);
        }
...
int main(int argc, int **argv) {
    buffer_index = 0;
    count = 0; // int val <- k in example code
 
    sem_init(&buffer_mutex,0, 1); // set bsem mutex to 1
    sem_init(&gate_mutex,0, 1); // set gate mutex to 1
